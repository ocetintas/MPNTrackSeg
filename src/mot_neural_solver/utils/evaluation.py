...
#from MOTChallengeEvalKit.src.MOTChallengeEvalKit.MOTS.evalMOTS import MOTS_evaluator
from MOTChallengeEvalKit.MOTS.evalMOTS import MOTS_evaluator
...
def compute_mots_metrics(gt_path, out_mots_files_path, seqs):
    evaluator = MOTS_evaluator()
    benchmark_name = "MOTS"
    gt_dir = gt_path
    res_dir = out_mots_files_path
    eval_mode = "train"
    ...
    dataframes = [seq.df for seq in sequence_metrics] + [overall_metrics.df]
    mots_summary = pd.concat(dataframes)
    return mots_summary
...
class MOTMetricsLogger(Callback):
    ...
    """
    def __init__(self, compute_oracle_results, compute_pred_oracles):
        super(MOTMetricsLogger).__init__()
        self.compute_oracle_results = compute_oracle_results
        self.compute_pred_oracles = compute_pred_oracles

    def _compute_mot_metrics(self, epoch_num, pl_module, oracle_results = False, pred_oracles=False):
        constr_satisf_rate = pl_module.track_all_seqs(dataset=self.dataset,
                                                      ...
                                                      pred_oracles=pred_oracles)

        # Compute MOTS Metrics
        mot_metrics_summary = compute_mots_metrics(gt_path=osp.join(DATA_PATH, self.dataset.seq_names[0][:-3]),
                                                   out_mots_files_path=self.output_files_dir,
                                                   seqs=self.dataset.seq_names)
        mot_metrics_summary['constr_sr'] = constr_satisf_rate
        mot_metrics_summary['epoch_num'] = epoch_num + 1

        if all('KITTI' in s for s in self.dataset.seq_names):  # Calculate HOTA if KITTI
            tracker_id = osp.split(osp.split(self.output_files_dir)[0])[1]  # Get the experiment id from the path
            # Copy the tracking output file with KITTI output name format
            for seq in self.dataset.seq_names:
                seq_num = int(seq[-2:])
                original_file = osp.join(self.output_files_dir, seq+'.txt')
                kitti_file = osp.join(self.output_files_dir, f'{seq_num:04}.txt')
                copyfile(original_file, kitti_file)
            eval_kitti_mots(tracker_id, split='val')
        ...
        if pred_oracles:
            ...
            print('GT EDGES')
            gt_edge_mot_summary = compute_mots_metrics(gt_path=osp.join(DATA_PATH, self.dataset.seq_names[0][:-3]),
                                                       out_mots_files_path=osp.join(self.output_files_dir,
                                                                                    'pred_oracles', 'gt_edge'),
                                                       seqs=self.dataset.seq_names)

            print('GT MASKS')
            gt_mask_mot_summary = compute_mots_metrics(gt_path=osp.join(DATA_PATH, self.dataset.seq_names[0][:-3]),
                                                       out_mots_files_path=osp.join(self.output_files_dir,
                                                                                    'pred_oracles', 'gt_mask'),
                                                       seqs=self.dataset.seq_names)
            print('###############################################')

            return mot_metrics_summary, gt_edge_mot_summary, gt_mask_mot_summary

        return mot_metrics_summary, None, None
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        if self.available_data:
            ...
            self.output_files_dir = osp.join(save_dir, 'mots_files')
            self.output_metrics_dir = osp.join(save_dir, 'mots_metrics')
        ...
        if self.available_data and self.compute_oracle_results:
            mot_metrics_summary, _, _ = self._compute_mot_metrics(trainer.current_epoch, pl_module, oracle_results=True)
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        if self.available_data:
            mot_metrics_summary, gt_edge_mot_summary, gt_mask_mot_summary = \
                self._compute_mot_metrics(trainer.current_epoch, pl_module, oracle_results=False,
                                          pred_oracles=self.compute_pred_oracles)
            ...
            if self.compute_pred_oracles:
                save_pickle(gt_edge_mot_summary.to_dict(),
                            osp.join(self.output_metrics_dir, f'epoch_{trainer.current_epoch + 1:03}_gt_edge.npy'))
                save_pickle(gt_mask_mot_summary.to_dict(),
                            osp.join(self.output_metrics_dir, f'epoch_{trainer.current_epoch + 1:03}_gt_mask.npy'))
