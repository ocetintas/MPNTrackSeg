...
class MOTGraphDataset:
    ...
    def _index_dataset(self):
        ...
        """
        frames_per_graph = self.dataset_params['frames_per_graph']
        min_detects = self.dataset_params['min_detects']
        assert isinstance(frames_per_graph, int) and frames_per_graph > 0
        assert isinstance(min_detects, int) and min_detects > 0
        ...
        for seq_name, det_df in self.seq_det_dfs.items():
            ...
            for start_frame in range(seq_step_size):
                # Consider all positions starting at start_frame and multiples of step_size
                sampled_frames_mask = (seq_det_df_['frame'] - start_frame).mod(seq_step_size) == 0
                sampled_seq_df = seq_det_df_[sampled_frames_mask]

                # Consider all such positions with enough detections to build a graph starting at them
                dets_per_graph = sampled_seq_df.groupby('frame')['bb_left'].count().reset_index()
                max_possible_f = dets_per_graph['frame'].max() - ((frames_per_graph-1) * seq_step_size)  # We want at least num_frames_per_graph
                valid_frames = []
                for f in dets_per_graph.frame:
                    if f <= max_possible_f:
                        valids = np.arange(f, f + (seq_step_size) * frames_per_graph, seq_step_size)  # Subset within the range
                        valids_df = dets_per_graph[dets_per_graph.frame.isin(valids)]
                        # Check if the total number of dets in that subset is satisfied. Also ensure that there are more than 1 frames (valid edges need to exist)
                        if valids_df['bb_left'].sum() >= min_detects and len(valids_df['frame'].unique()) > 1:
                            valid_frames.append(f)

                seq_frame_ixs.extend([(seq_name, frame) for frame in valid_frames])
